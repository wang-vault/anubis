...
/**
 * Halaman Audio Extractor — fitur PUBLIK terpisah dari toko (tanpa login,
 * tanpa menyentuh database). Semua pengambilan data lewat /api/youtube/audio
 * yang sudah dibatasi rate limit per IP.
 *
 * Alur: user menempel link video/short YouTube → GET /api/youtube/audio?url=… →
 * tampilkan tombol unduh audionya (MP3, agar bisa diputar di semua perangkat).
 */
...
export default function AudioExtractorPage() {
  ...
  const [result, setResult] = useState<AudioMedia | null>(null);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/api/youtube/audio?url=${encodeURIComponent(trimmed)}`);
  ...
  return (
    <div className="container-x max-w-3xl">
      ...
      <div className="paper-heading">
        ...
        <h1 className="paper-heading-title">Audio Extractor</h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          Ekstrak audio dari video YouTube dalam format MP3. Gratis dan tanpa login.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card mt-6 p-5" noValidate>
        ...
        <p id="audio-url-hint" className="hint">
      ...
      {loading && (
        <div className="card mt-6 p-5" aria-busy="true" aria-live="polite">
          <p className="text-sm font-bold">Mengambil tautan audio dari YouTube…</p>
      ...
      {result && (
        <article className="card mt-6 p-5" aria-live="polite">
          ...
          <p className="mt-1.5 text-sm leading-6 text-slate-600">
            Tekan tombol di bawah untuk mengunduh audionya (MP3).
          </p>
          <div className="mt-4">
            <a
              href={`/api/youtube/audio/download?url=${encodeURIComponent(result.url)}&filename=${encodeURIComponent(result.filename)}`}
              download={result.filename}
              className="btn-primary"
              title={`Unduh audio: ${result.filename}`}
            >
              ↓ Download MP3
            </a>
          </div>
