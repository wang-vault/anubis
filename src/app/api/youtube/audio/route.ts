 ...
 *
 * Fitur PUBLIK (tanpa login): ambil tautan unduh AUDIO dari video/short
 * YouTube lewat Cobalt instance self-hosted (di Railway), lalu kembalikan
 * tautan audio + nama file-nya ke browser. Format MP3 diminta eksplisit agar
 * hasil bisa diputar di (hampir) semua perangkat.
 *
...
/** Susun respons final; wajib ada tautan audio (status tunnel/redirect). */
function toMedia(body: CobaltResponse): YouTubeMedia {
  ...
  return {
    url: body.url,
    filename: typeof body.filename === "string" && body.filename ? body.filename : "audio.mp3",
  };
}
