 ...
 *
 * Toko ini dijalankan "manual saja": QRIS otomatis dimatikan dengan
 * mengosongkan kredensial YoBasePay. Yang diuji di sini adalah pemisahan
...
beforeEach(() => {
  ...
  delete process.env.YOBASEPAY_API_KEY;
  delete process.env.YOBASEPAY_WEBHOOK_SECRET;
});
...
describe("DEFAULT_PAYMENT_METHOD — preferensi, bukan kredensial", () => {
  it("menerima variasi kapitalisasi & spasi", async () => {
    ...
    expect(serverEnv().DEFAULT_PAYMENT_METHOD).toBe("YOBASEPAY");
  });
...
describe("mode manual-saja", () => {
  it("YoBasePay nonaktif selama kredensialnya kosong", async () => {
    const { serverEnv, yobasepayConfigured } = await loadEnv({});
    expect(yobasepayConfigured(serverEnv())).toBe(false);
  });

  it("butuh API key DAN webhook secret — setengah terisi tetap nonaktif", async () => {
    const onlyKey = await loadEnv({ YOBASEPAY_API_KEY: "key-123" });
    expect(onlyKey.yobasepayConfigured(onlyKey.serverEnv())).toBe(false);

    const onlySecret = await loadEnv({ YOBASEPAY_WEBHOOK_SECRET: "secret-123" });
    expect(onlySecret.yobasepayConfigured(onlySecret.serverEnv())).toBe(false);

    const both = await loadEnv({
      YOBASEPAY_API_KEY: "key-123",
      YOBASEPAY_WEBHOOK_SECRET: "secret-123",
    });
    expect(both.yobasepayConfigured(both.serverEnv())).toBe(true);
  });
});
